...
//-----------------------------------------------------------------------------
FiniteElement::FiniteElement(const fenics_finite_element& fenics_element)
    : _signature(fenics_element.signature), _family(fenics_element.family),
      _tdim(fenics_element.topological_dimension),
      _space_dim(fenics_element.space_dimension),
      _value_size(fenics_element.value_size),
      _reference_value_size(fenics_element.reference_value_size),
      _degree(fenics_element.degree), _hash(std::hash<std::string>{}(_signature)),
      _evaluate_reference_basis(fenics_element.evaluate_reference_basis),
      _evaluate_reference_basis_derivatives(
          fenics_element.evaluate_reference_basis_derivatives),
      _transform_reference_basis_derivatives(
          fenics_element.transform_reference_basis_derivatives),
      _transform_values(fenics_element.transform_values)
{
  // Store dof coordinates on reference element
  assert(fenics_element.tabulate_reference_dof_coordinates);
  _refX.resize(_space_dim, _tdim);
  if (fenics_element.tabulate_reference_dof_coordinates(_refX.data()) == -1)
  {
    throw std::runtime_error(
        "Generated code returned error in tabulate_reference_dof_coordinates");
  }

  const fenics_shape _shape = fenics_element.cell_shape;
  switch (_shape)
  {
  case interval:
  ...
  default:
    throw std::runtime_error(
        "Unknown FEniCS cell type when building FiniteElement.");
  }
  assert(mesh::cell_dim(_cell_shape) == _tdim);

  // Fill value dimension
  for (int i = 0; i < fenics_element.value_rank; ++i)
    _value_dimension.push_back(fenics_element.value_dimension(i));

  // Create all sub-elements
  for (int i = 0; i < fenics_element.num_sub_elements; ++i)
  {
    fenics_finite_element* fenics_sub_element = fenics_element.create_sub_element(i);
    _sub_elements.push_back(std::make_shared<FiniteElement>(*fenics_sub_element));
    std::free(fenics_sub_element);
  }
}
//-----------------------------------------------------------------------------
