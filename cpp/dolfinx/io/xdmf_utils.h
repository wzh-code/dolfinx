...
/// Received owned entities and associated data from input entities on
/// other ranks and defined by global 'node' indices. The input entities
/// and data can be supplied on any rank and this function will manage
/// the communication. It it typically used for distributing data that
/// is attached to a mesh entity.
///
/// @param[in] mesh A mesh
/// @param[in] entity_dim Topological dimension of the mesh entities
...
/// triangle (using local (to the MPI rank) indexing). Each vertex has a
/// 'node' (geometry dof) index, and each node has a persistent input
/// global index allowing the triangle [gi0, gi1, gi2] to be identified
/// with [v0, v1, v2].
/// @param[in] data Data associated with each entity in `entities`.
/// @return (Cell-vertex connectivity of entities owned by this rank ,
/// associated data (values) with each received entity)
///
...
/// owned by the calling rank can be passed to this function. E.g.,
/// rank0 provides an entity with global input indices [gi0, gi1, gi2],
/// but this may be a triangle that is owned by rank1. The function will
/// send to rank1 the (local) cell-vertex connectivity for this
/// triangle and the attached data.
